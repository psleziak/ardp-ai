...
- **Exception:** thin persistence/storage accessors are allowed here even though they are not pure -
  e.g. small `readXxx`/`writeXxx` wrappers around `localStorage`/`sessionStorage` plus their key
  constant. Keep them minimal; anything heavier belongs in a service or the `*.api.ts`.
...
- Pass dependencies in as parameters. For i18n, pass the translations object in
  (`trans: typeof trans` from `@icz/core/shared/ui-core`) rather than importing state.
